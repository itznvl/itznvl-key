"""
ITZNVL KEY SYSTEM — Flask web service cho Render
- Landing page + GETKEY
- Admin panel tạo key có hạn
- Anti-bypass: HMAC sign, HWID bind, rate limit, fingerprint
"""
import os, time, secrets, hmac, hashlib, sqlite3, threading
from functools import wraps
from flask import (Flask, request, jsonify, render_template_string,
                   redirect, session, make_response, abort)
...
HMAC_SECRET    = os.environ.get("HMAC_SECRET", "itznvl-hmac-secret-change-me")
KEY_DAYS       = int(os.environ.get("KEY_DAYS", "1"))
DB_PATH        = os.environ.get("DB_PATH", "keys.db")
RATE_LIMIT_MAX = int(os.environ.get("RATE_LIMIT_MAX", "5"))       # 5 lần/giờ/IP
...
def init_db():
    with db_lock:
        ...
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_hwid ON keys(hwid)")
        c.commit()
        c.close()
...
def sign_payload(data: dict) -> str:
    """Ký payload bằng HMAC — client phải verify, chống bypass bằng sửa response."""
...
def fingerprint(hwid, ua, ip):
    """Fingerprint = hash(HWID + UserAgent + IP_class)"""
    ip_class = ".".join(ip.split(".")[:2]) if "." in ip else ip
    raw = f"{hwid}|{ua}|{ip_class}"
    return hashlib.sha256(raw.encode()).hexdigest()[:32]

def rate_check(ident):
    """Trả True nếu vượt rate limit."""
    now = int(time.time())
    with db_lock:
        c = db()
        c.execute("DELETE FROM rate WHERE ts < ?", (now - RATE_LIMIT_WIN,))
        cnt = c.execute("SELECT COUNT(*) FROM rate WHERE ident=?", (ident,)).fetchone()[0]
        if cnt >= RATE_LIMIT_MAX:
            c.close()
            return True
        c.execute("INSERT INTO rate (ident, ts) VALUES (?, ?)", (ident, now))
        c.commit()
        c.close()
    return False

def add_keys(n, days=None, note=None, prefix="ITZ"):
    created = []
    with db_lock:
        c = db()
        for _ in range(n):
            k = gen_key(prefix)
            try:
                c.execute("INSERT INTO keys (key, note) VALUES (?, ?)", (k, note or ""))
                created.append(k)
...
.bg{
  ...
  background:
    radial-gradient(circle at 20% 10%, rgba(0,180,255,.18), transparent 45%),
    radial-gradient(circle at 85% 80%, rgba(120,0,255,.15), transparent 45%),
    radial-gradient(circle at 50% 50%, rgba(0,80,140,.08), transparent 65%),
    #05070d;
}
.bg::before{
...
.logo{
  ...
  font-size:13px;font-weight:600;letter-spacing:2px;color:#00d9ff;
  margin-bottom:32px;
}
.logo .dot{width:8px;height:8px;border-radius:50%;background:#00ff88;box-shadow:0 0 12px #00ff88;animation:pulse 2s infinite}
@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.8)}}
h1{
  font-size:clamp(38px,8vw,64px);font-weight:900;line-height:1.05;
...
.card{
  background:linear-gradient(160deg,rgba(10,20,35,.9),rgba(5,10,20,.95));
...
.label{font-size:12px;letter-spacing:2px;color:#00d9ff;font-weight:700;margin-bottom:12px;text-transform:uppercase}
.hwid{
  font-family:"SF Mono",Menlo,Consolas,monospace;font-size:13px;color:#8ba3b8;
...
@keyframes btnPulse{
  0%,100%{box-shadow:0 12px 40px rgba(0,120,255,.4)}
  50%{box-shadow:0 12px 55px rgba(0,200,255,.7),0 0 0 8px rgba(0,180,255,.06)}
}
.key-box{
  background:linear-gradient(135deg,rgba(0,255,136,.08),rgba(0,180,255,.08));
  border:2px solid #00ff88;border-radius:16px;padding:26px 20px;text-align:center;
  margin:20px 0;
  box-shadow:0 0 40px rgba(0,255,136,.15),inset 0 1px 0 rgba(255,255,255,.05);
  animation:rise .5s cubic-bezier(.2,.8,.3,1) forwards;
}
.key-val{
  font-family:"SF Mono",Menlo,Consolas,monospace;font-size:clamp(20px,5vw,28px);
  font-weight:800;color:#00ff88;letter-spacing:2px;word-break:break-all;
  text-shadow:0 0 20px rgba(0,255,136,.5);
}
.copy-row{display:flex;gap:8px;margin-top:16px}
.copy-btn{
  flex:1;padding:14px;border-radius:10px;border:1px solid rgba(0,255,136,.4);
...
.meta-item .k{font-size:11px;color:#4a5f75;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px}
.meta-item .v{font-size:15px;color:#e6f6ff;font-weight:700}
.steps{counter-reset:s;list-style:none}
.steps li{
  counter-increment:s;padding:14px 0 14px 44px;position:relative;
...
.err{
  ...
  color:#ff8b8b;padding:16px;border-radius:10px;font-size:14px;
  text-align:center;
}
.toast{
...
function copyKey(){
  ...
  navigator.clipboard.writeText(k).then(()=>{
    ...
    document.querySelector('.copy-btn').classList.add('done');
    document.querySelector('.copy-btn').textContent = '✓ ĐÃ SAO CHÉP';
  });
}
</script>
</body>
...
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#05070d;color:#e6f6ff;min-height:100vh}
.bg{position:fixed;inset:0;z-index:0;background:radial-gradient(circle at 15% 5%,rgba(0,180,255,.15),transparent 40%),radial-gradient(circle at 85% 95%,rgba(120,0,255,.12),transparent 40%),#05070d}
...
h1{font-size:32px;font-weight:900;letter-spacing:-1px;margin-bottom:6px;
   background:linear-gradient(135deg,#fff,#00d9ff);-webkit-background-clip:text;color:transparent}
...
table{width:100%;border-collapse:collapse;font-size:13px;font-family:ui-monospace,Menlo,monospace}
...
def getkey():
    ...
    if not hwid:
        return render_template_string(LANDING,
            hwid="", fp="", key=None, days=KEY_DAYS, expiry_str="",
    ...
    if rate_check(f"getkey:{ip}"):
        return render_template_string(LANDING,
            hwid=hwid, fp=fp, key=None, days=KEY_DAYS, expiry_str="",
            error="Bạn đã thử quá nhiều lần. Vui lòng đợi 1 giờ.")

    with db_lock:
        c = db()
        # 1. HWID đã có key chưa?
        row = c.execute(
            "SELECT key, expiry FROM keys WHERE hwid=? ORDER BY claimed_at DESC LIMIT 1",
            (hwid,)
        ).fetchone()
        if row:
            c.close()
            ...
            return render_template_string(LANDING,
                hwid=hwid, fp=fp, key=row["key"], days=KEY_DAYS,
                expiry_str=exp_str, error=None)

        # 2. POST → claim
        if request.method == "POST":
            # Kiểm tra fingerprint match (chống copy link)
            row = c.execute(
                "SELECT key FROM keys WHERE hwid IS NULL OR hwid='' ORDER BY rowid ASC LIMIT 1"
            ).fetchone()
            if not row:
                c.close()
                return render_template_string(LANDING,
            ...
            c.execute(
                "UPDATE keys SET hwid=?, fingerprint=?, activated=?, expiry=?, claimed_at=?, ip=? WHERE key=?",
                (hwid, fp, now, expiry, now, ip, key)
            )
            c.commit()
            c.close()
            exp_str = datetime.fromtimestamp(expiry).strftime("%d/%m/%Y %H:%M")
            return render_template_string(LANDING,
                hwid=hwid, fp=fp, key=key, days=KEY_DAYS,
                expiry_str=exp_str, error=None)

        c.close()

    return render_template_string(LANDING,
        hwid=hwid, fp=fp, key=None, days=KEY_DAYS, expiry_str="", error=None)

# ============================================================
# ROUTES — VERIFY (dylib gọi)
# ============================================================
...
def verify():
    ...
    with db_lock:
        c = db()
        row = c.execute(
            "SELECT hwid, fingerprint, activated, expiry FROM keys WHERE key=?",
            (key,)
        ).fetchone()
        if not row:
            c.close()
        ...
        # Fingerprint mismatch (IP class or UA đổi nhiều) — cảnh báo nhưng vẫn cho qua lần đầu
        if activated and saved_fp and saved_fp != cur_fp:
            # cho phép nếu cùng HWID nhưng đổi mạng — chỉ log
            pass
        ...
        if not activated:
            expiry = now + KEY_DAYS * 86400
            c.execute(
                "UPDATE keys SET hwid=?, fingerprint=?, activated=?, expiry=?, ip=? WHERE key=?",
                (hwid, cur_fp, now, expiry, ip, key)
            )
            c.commit()
...
def admin():
    ...
    with db_lock:
        c = db()
        rows = c.execute(
            "SELECT key, hwid, activated, expiry, note, ip FROM keys "
            "ORDER BY claimed_at DESC, key ASC"
        ).fetchall()
        c.close()
    ...
    for r in rows:
        if not r["activated"]:
            status, cls = "free", "free"
            free += 1
            exp_str = "—"
        elif now > r["expiry"]:
            status, cls = "expired", "exp"
            expired += 1
            exp_str = datetime.fromtimestamp(r["expiry"]).strftime("%d/%m %H:%M")
        else:
            status, cls = "used", "used"
            used += 1
            exp_str = datetime.fromtimestamp(r["expiry"]).strftime("%d/%m %H:%M")
        keys_out.append({
...
def admin_clear():
    ...
    with db_lock:
        c = db()
        cur = c.execute("DELETE FROM keys WHERE activated=0")
        n = cur.rowcount
        c.commit()
        c.close()
